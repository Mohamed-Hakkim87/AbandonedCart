...
(function ($) {
    ...
    $.stickToMe = function ( configs ) {
        ...
        var reqsettings = {
            ...
            boxcss: {'z-index':'1000','position':'fixed','left':'0','top':'40%','right': '0','margin': '0 auto'}
        };
        ...
        function chromefix()
        {
            offsetbind = false;
            $(document).bind(
                'mousemove.bindoffset',function (e) {
                    if (offsetbind) {
                        $(document).bind(
                            'mouseleave', function (e) {
                                setTimeout(
        ...
        function showbox()
        {
            if ($.data(document.body, "stick_var") != 1) {
                ...
                $('<div class="hikmadh_abandoned_cart_stick_block_layer"></div>').appendTo('body').css(settings.backgroundcss).fadeIn(settings.fadespeed);
                ...
                if (settings.bgclickclose) {
                    $('.hikmadh_abandoned_cart_stick_block_layer').click(
                        function () {
                            stick_close();
                        }
                    );
        ...
        function mobilePopup() {
            if(windowWidth < 1024 && settings.layer != ""){
                showbox();
            }
        }
    };
    ...
    {
        $('.stick_container').fadeOut(
            function () {
                $(this).remove(); }
        );
        $('.hikmadh_abandoned_cart_stick_block_layer').fadeOut(
            function () {
                $(this).remove(); }
        );
